...
unsigned CPUWriteState(uint8_t* data, unsigned size)
{
	...
	utilWriteMem(data, systemGetPixels(), systemGetScreenDepth() * systemGetScreenWidth() * systemGetScreenHeight());
...
void CPUCleanUp (void)
{
	...
	if(graphics.paletteRAM != NULL) {
		systemFree(graphics.paletteRAM);
		graphics.paletteRAM = NULL;
	}

	if(internalRAM != NULL) {
		systemFree(internalRAM);
		internalRAM = NULL;
	}
...
int CPULoadRom(const char * file)
{
	...
	if(bios == NULL) {
		systemMessage("Could not allocate memory for BIOS.");
		CPUCleanUp();
		return 0;
	}
	internalRAM = (uint8_t *)systemAlloc(0x8000);
	if(internalRAM == NULL) {
		systemMessage("Could not allocate memory for Internal RAM.");
		CPUCleanUp();
		return 0;
	}
	graphics.paletteRAM = (uint8_t *)systemAlloc(0x400);
	if(graphics.paletteRAM == NULL) {
		systemMessage("Could not allocate memory for Palette RAM.");
		CPUCleanUp();
		return 0;
	}
	vram = (uint8_t *)systemAlloc(0x20000);
	if(vram == NULL) {
		systemMessage("Could not allocate memory for VRAM.");
		CPUCleanUp();
		return 0;
	}
	oam = (uint8_t *)systemAlloc(0x400);
	if(oam == NULL) {
		systemMessage("Could not allocate memory for OAM.");
		CPUCleanUp();
		return 0;
	}
	ioMem = (uint8_t *)systemAlloc(0x400);
	if(ioMem == NULL) {
		systemMessage("Could not allocate memory for I/O.");
		CPUCleanUp();
		return 0;
	}
...
bool CPUReadState(const uint8_t* data, unsigned size)
{
	...
	utilReadMem(systemGetPixels(), data, systemGetScreenDepth() * systemGetScreenWidth() * systemGetScreenHeight());
...
void CPUReset (void)
{
	...
	memset(systemGetPixels(), 0, systemGetScreenDepth() * systemGetScreenWidth() * systemGetScreenHeight());			// clean picture
